package br.jus.tre_pa.jfilter.sql.mysql;
...
public class MySQLWhereStep implements SqlWhereStep {

	private JdbcTemplate jdbcTemplate;

	private Map<String, String> sqlFragments = new HashMap<>();

	public MySQLWhereStep(JdbcTemplate jdbcTemplate, Map<String, String> sqlFragments) {
		super();
	...
	public SqlPaginationStep orderBy(String sortSql) {
		...
		return new MySQLPaginationStep(jdbcTemplate, sqlFragments);
	}
	...
	public SqlOrderByStep where(Filterable filter) {
		...
		return new MySQLOrderByStep(jdbcTemplate, sqlFragments);
	}
